...
function hideLegenda() {
  ...
  $('#tombol').append('<a type="button" id="showlegenda" onclick="legenda()" class="btn btn-success " data-toggle="tooltip" title="Legenda" style="margin-right: 7px;color:black;"><i class="fa fa-eye" style="color:black;"> </i></a>');
}
      ...
      function industri_sekitar(latitude,longitude,rad){ // INDUSTRI SEKITAR
        ...
        $.ajax({url: server+'_sekitar_small_industry.php?lat='+latitude+'&lng='+longitude+'&rad='+rad, data: "", dataType: 'json', success: function(rows){ 
          for (var i in rows){ 
            ...
            klikInfoWindowSM(id,marker);
            map.setCenter(centerBaru);
            $('#table_kanan_industry').append("<tr><td>"+name+"</td><td><a role='button' style='margin:5px' class='btn btn-success fa fa-road' onclick='route_sekitar(\""+latitude+"\",\""+longitude+"\",\""+lat+"\",\""+lon+"\")'></a><a role='button' style='margin:5px' class='btn btn-success fa fa-info' onclick='modal_small_industry(\""+id+"\")'></a><a role='button' style='margin:5px' class='btn btn-success fa fa-map-marker' onclick='set_center(\""+lat+"\",\""+lon+"\",\""+name+"\")'></a><a role='button' class='btn btn-success fa fa-bus' style='margin:5px' onclick='angkot_sekitar_lagi(\""+id+"\")'></a></td></tr>");
          }//end for
        }});//end ajax  
      }
      ...
      function angkot_sekitar(id){ // Menu Angkot - List Angkot
        ...
        { 
            for (var i in rows){ 
              ...
              $('#table_kanan_angkot').append("<tr><td>"+name+"</td><td><a role='button' style='margin:5px' class='btn btn-danger fa fa-road' onclick='route_sekitar(\""+latitude+"\",\""+longitude+"\",\""+lat+"\",\""+lon+"\")'></a><a role='button' style='margin:5px' class='btn btn-success fa fa-info' onclick='modal_small_industry(\""+id+"\")'></a><a role='button' style='margin:5px' class='btn btn-success fa fa-map-marker' onclick='set_center(\""+lat+"\",\""+lon+"\",\""+name+"\")'></a><a role='button' class='btn btn-danger fa fa-bus' style='margin:5px' onclick='angkot_sekitar_lagi(\""+id+"\")'></a></td></tr>");
          }               
      ...
      function angkot_sekitar_lagi(id){ // Menu Angkot - List Angkot
        ...
        { 
            for (var i in rows){ 
              ...
              console.log(id);
              //route_angkot_1(id,color);
              $('#view_tracking_table').append("<tr><td>"+destination+"</td><td><a role='button' class='btn btn-success fa fa-info' onclick='modal_angkot(\""+ids+"\")'></a><a style='margin-left:10px' role='button' class='btn btn-danger fa fa-bus' onclick='clearangkot();route_angkot(\""+lat2+"\",\""+lng2+"\",\""+lat+"\",\""+lng+"\",\""+ids+"\",\""+id+"\")'></a></td></tr>");         
            }               
        }});      
      }
